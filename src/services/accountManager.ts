 ...
 * Account management for DeepSeek Gateway.
...
const ACCOUNTS_FILE = projectPath(".deepseek", "accounts.json");
const FALLBACK_ACCOUNTS_FILE = projectPath(".deepseek", "accounts.jsonc");
const DEEPSEEK_DIR = projectPath(".deepseek");

const OLD_ACCOUNTS_FILE = projectPath("deepseek_profile", "accounts.json");
...
export function migrateFromOldPaths(): void {
  try {
    ...
    logStore.log("info", "auth", "Migrating data from deepseek_profile/ to .deepseek/ ...");
    ...
    logStore.log("info", "auth", "Migrated accounts.json from deepseek_profile/ to .deepseek/");
...
const IV_LENGTH = 16;
const MASTER_KEY_FILE = projectPath(".deepseek", "master.key");
...
function deriveKey(keyMaterial: string): Buffer {
  return crypto.scryptSync(keyMaterial, "deepseek-gate-salt", 32);
}
...
// O(1) email→account lookup index
...
export function saveAccountsToFile(accts: readonly AccountEntry[]): void {
  const dir = path.dirname(ACCOUNTS_FILE);
  if (!existsSync(dir)) {
    mkdirSync(dir, { recursive: true });
  }
  const data: PersistedAccountData[] = accts
    .filter((a) => a.password)
    .map((a) => ({
      email: a.email,
      password: encryptPassword(a.password!),
      ...(a.throttledUntil && a.throttledUntil > Date.now() ? { throttledUntil: a.throttledUntil } : {}),
...
export function loadAccountsFromFile(): Array<{
  ...
  const tryLoad = (
    filePath: string,
  ): Array<{ email: string; password: string; throttledUntil?: number; disabled?: boolean }> | null => {
    try {
...
export async function addAccount(
  ...
  if (existing) {
    return { loginSucceeded: false, loginError: "Account already exists" };
  }
...
export async function removeAccount(email: string): Promise<void> {
  ...
  if (existsSync(profileDir)) {
    try {
      ...
      logStore.log("error", "auth", `Failed to delete browser profile for ${normalizedEmail}: ${err.message}`);
    }
  }
}
...
export function setupAccountWatcher(): void {
  ...
  if (!existsSync(DEEPSEEK_DIR)) {
    mkdirSync(DEEPSEEK_DIR, { recursive: true });
  }
  try {
    accountWatcher = watch(DEEPSEEK_DIR, (_eventType: string, filename: string | null) => {
...
export function isAvailable(acct: AccountEntry): boolean {
  ...
  if (acct.throttledUntil && acct.throttledUntil > Date.now()) return false;
  return true;
}
...
export async function pickAccount(excludeEmail?: string): Promise<AccountEntry | null> {
  try {
    ...
    if (available.length === 0) {
      ...
      const throttled = accounts.filter((a) => a.throttledUntil && a.throttledUntil > Date.now()).length;
      const noState = accounts.filter((a) => !a.state).length;
      logStore.log(
        "warn",
        "auth",
        `All ${accounts.length} accounts exhausted — ${throttled} throttled, ${noState} unauthenticated`,
      );
      return null;
    }
...
export function isAccountThrottled(email: string): boolean {
  const acct = getAccountByEmail(email);
  if (!acct) return true;
  return Boolean(acct.throttledUntil && acct.throttledUntil > Date.now());
}
...
export function getAccountStats(): Array<{
  ...
  return accounts.map((a) => ({
    ...
    throttled: Boolean(a.throttledUntil && a.throttledUntil > now),
    disabled: a.disabled ?? false,
    throttledRemainingMs: a.throttledUntil ? Math.max(0, a.throttledUntil - now) : 0,
    throttledUnlockAt: a.throttledUntil && a.throttledUntil > now ? new Date(a.throttledUntil).toISOString() : null,

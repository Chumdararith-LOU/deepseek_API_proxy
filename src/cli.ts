...
function err(msg: string) {
  console.error(`[dsg] ${msg}`);
}
const DEFAULT_PORT = "26406";

function showHelp() {
  out("");
  out("DeepSeek Gateway — OpenAI-compatible gateway for DeepSeek AI");
  out("");
  out("USAGE");
  out("  dsg [command] [options]");
  out("");
  out("COMMANDS");
  ...
  out("");
  out("OPTIONS");
  out("  --port <n>     Override port (default: from config or 26406)");
  out("  --browser <e>  Browser engine: chromium, firefox, chrome, edge");
  out("  --host <addr>  Bind address (default: from config or localhost)");
  out("");
  out("EXAMPLES");
  out("  dsg                    Start the server");
  out("  dsg update             Update to latest version");
  out("  dsg start --port 8080  Start on port 8080");
  out("  dsg restart            Restart the server");
  out("  dsg status             Check server status");
  out("  dsg help               Show this message");
  out("");
  out("ACCOUNT MANAGEMENT");
  out("  Use the web dashboard at http://localhost:26406/dashboard/accounts");
  out("  to add, remove, and manage your DeepSeek accounts.");
  out("");
}

function findEntry(): string {
  if (existsSync(SERVER_ENTRY)) return SERVER_ENTRY;
...
async function startServer(args: string[]) {
  ...
  if (portIdx !== -1 && args[portIdx + 1]) spawnEnv.PORT = args[portIdx + 1];
  if (hostIdx !== -1 && args[hostIdx + 1]) spawnEnv.HOST = args[hostIdx + 1];
...
async function doUpdate() {
  ...
  if (isBun) {
    ...
    if (bunCode === 0) {
      out("Update complete. Restart the server with: dsg restart");
      return;
    }
...
function getPidFilePath(): string {
  return resolve(__dirname, "..", ".deepseek", "gate.pid");
}
...
if (
  process.argv[1] &&
  (fileURLToPath(import.meta.url).endsWith("cli.ts") || fileURLToPath(import.meta.url).endsWith("cli.js"))
) {
  main().catch((e) => {
    err(`Fatal: ${e.message}`);
    process.exit(1);

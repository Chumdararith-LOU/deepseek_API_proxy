  ...
  while (true) {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        console.error(
          "[Chat] Streaming: parse error on chunk, ignoring partial:",
          (e as Error)?.message,
          "raw:",
          dataStr.slice(0, 200),
        );
...
export async function handlePostStreamCompletion(
  ...
  try {
        ...
        if (
          ...
          if (ct) {
            ...
            for (let i = 0; i < ct.length; i += 256) {
              await writeEvent(
                streamWriter,
                buildChunkEvent(completionId, model, [makeChoice({ content: ct.slice(i, i + 256) })]),
              );
              await new Promise((r) => setTimeout(r, 0));
            }
          }
    ...
    const upstreamError =
      parseDeepseekErrorPayload(buffer) ||
      (streamState.upstreamError ? { message: streamState.upstreamError, status: 502 as const } : null);
    if (upstreamError) {

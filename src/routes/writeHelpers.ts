...
export function buildChunkEvent(
  ...
  return {
    ...
    model,
    system_fingerprint: "fp_deepseek_gate",
...
export async function writeContentDelta(
  ...
  lastVStrRaw: string,
  logStore: {
    addProcessedOutput: (id: string, c: string) => void;
    updateEntry: (id: string, fn: (e: any) => void) => void;
  },
): Promise<boolean> {
  if (
    checkAmplificationGuard(ampState, contentDelta.length, logId, resolvedEmail, model, lastRawContent, lastVStrRaw)
  ) {
    return false;
  }

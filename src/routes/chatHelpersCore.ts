...
import { DEEPSEEK_THINK_TAG_PATTERN as THINK_TAG_PATTERN } from "../utils/thinkTagStripper.ts";
...
export function detectCumulativeChunk(newText: string, lastText: string): { cumulative: boolean; delta: string } {
  ...
  // Fingerprint-based recovery: DeepSeek sometimes resends cumulative content
 ...
 * DeepSeek's tool call format uses `<keyword=value>` and `</keyword>` patterns
 ...
 *
 * Add new keywords to the array when DeepSeek's tool call format changes.
 */
...
export function parseDeepseekErrorPayload(
  raw: string,
  ...
  try {
    ...
    if (payload && payload.success === false) {
      ...
      const details = payload.data?.details || payload.message || "Deepseek returned an error";
      const wait =
        payload.data?.num !== undefined ? ` Wait about ${payload.data.num} hour(s) before trying again.` : "";
      const status = code === "RateLimited" ? 429 : code === "Not_Found" ? 404 : 502;
      return { message: `Deepseek upstream error: ${code}: ${details}.${wait}`, status };
    }
    if (payload && payload.error) {
      const msg =
        typeof payload.error === "string" ? payload.error : payload.error.message || JSON.stringify(payload.error);
      return { message: `Deepseek upstream error: ${msg}`, status: 502 };
    }
  } catch {
    return null;
  }
...
export function extractDeltaContent(
  ...
  // ── JSON-Patch format (DeepSeek native SSE) ──────────────────────
  // Frames: {"p":"response/content","o":"APPEND","v":"text"} or implicit {"v":"text"}
  if (!chunk.choices && (chunk.p !== undefined || chunk.o !== undefined || chunk.v !== undefined)) {
    const v = chunk.v;
    ...
    if (v && typeof v === "object" && !Array.isArray(v)) {
      ...
      if (Array.isArray(frags)) {
        for (const frag of frags) {
          if (frag && frag.type === "RESPONSE" && typeof frag.content === "string" && frag.content) {
            newActivePath = "response/fragments/-1/content";
            vStr = frag.content;
            foundStr = true;
          }
        }
    ...
    // APPEND frame
    if (chunk.o === "APPEND") {
      if (typeof chunk.p === "string" && chunk.p) newActivePath = chunk.p;
      if (typeof v === "string" && newActivePath && /content$/.test(newActivePath)) {
        vStr = v;
        foundStr = true;
      }
      return {
    ...
    // Non-append frame with explicit path (status, token_usage, etc.)
    if (typeof chunk.p === "string" && chunk.p) {
      newActivePath = chunk.p;
      // Detect FINISHED status
      if (chunk.p === "response/status" && v === "FINISHED") {
        streamFinished = true;
      }
      return {
        vStr: "",
    ...
    // Bare value frame appending to active content path
    if (typeof v === "string" && newActivePath && /content$/.test(newActivePath)) {
      vStr = v;
      foundStr = true;
    }
  ...
  // ── OpenAI-compatible format (choices[0].delta) ──────────────────
  if (
    chunk.choices &&
    chunk.choices[0] &&
    ...
    (targetResponseId === null ||
      ...
      chunk["response.created"]?.response_id === targetResponseId)
  ) {

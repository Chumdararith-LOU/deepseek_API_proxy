...
import { configureAccount, deleteAllChats } from "../../services/deepseek.ts";
    ...
    async start(controller) {
      ...
      for (const email of emails) {
        try {
          controller.enqueue(
            encoder.encode(
              `data: ${JSON.stringify({ type: "progress", email: maskEmail(email), status: "deleting" })}\n\n`,
            ),
          );
          ...
          controller.enqueue(
            encoder.encode(
              `data: ${JSON.stringify({ type: "progress", email: maskEmail(email), status: "done" })}\n\n`,
            ),
          );
        } catch (err: any) {
...
function systemLogsHandler(c: any) {
  ...
  const logs = logStore.getLogs(limit, category, minLevel);
  return c.json(logs.map(sanitizeLogEntry));
}

function modelHealthHandler(c: any) {
  const entries = logStore.getAll();
  const byModel = new Map<string, { total: number; errors: number; latencies: number[] }>();
  for (const e of entries) {
    const m = byModel.get(e.model) || { total: 0, errors: 0, latencies: [] };
    m.total++;
    if (e.errors.length > 0) m.errors++;
    if (e.latency_ms != null) m.latencies.push(e.latency_ms);
...
function logJsonHandler(c: any) {
  ...
  const entries = logStore.getEntries(Math.max(1, Math.min(limit, 500)));
  const serialized = entries.map((e: any) => {
    const toolCalls = (e.parsedToolCalls || []).map((tc: any) => {
    ...
    return {
      ...
      thinking_content: e.reasoningContent || e.reasoning_content || "",
      raw_output: e.rawResponse || "",
      proccessed_output: e.processedResponse || "",

...
import type { StreamProcessingCtx, StreamProcessingState } from "./chatStreamingHelpers.ts";
...
export async function handleStreamingRequest(ctx: StreamingContext): Promise<Response> {
  ...
  return honoStream(c, async (streamWriter: any) => {
    ...
    logStore.log(
      "debug",
      "stream",
      `[Stream] >>> Streaming started for ${logId}, model=${body.model}, tools=${body.tools?.length || 0}`,
    );
    ...
    try {
      ...
      await writeEvent(
        streamWriter,
        buildChunkEvent(completionId, body.model, [makeChoice({ role: "assistant", content: "" })]),
      );
      ...
      const streamCtx: StreamProcessingCtx = {
        ...
        ampState,
        deepseekAbortController,
        deepseekLogFile: ctx.deepseekLogFile,
      ...
      if (loopResult.error) {
        ...
        // leaks in the pool and the chat persists on DeepSeek's servers indefinitely.
        cleanupImmediately(
          streamReader,
          heartbeatInterval,
          session.chatId,
      ...
      logStore.log(
        "debug",
        "stream",
        `[Stream] <<< Streaming completed for ${logId} in ${Date.now() - _streamStartTime}ms`,
      );
    } finally {
      if (!streamReleased) {

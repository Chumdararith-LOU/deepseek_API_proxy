...
function compressCargoTest(content: string, lines: string[], totalLines: number): string | null {
  if (
    ...
    !(lines.some((l) => /^test\s+\S+.*\s(ok|FAILED)\s/.test(l)) || content.includes("test result:"))
  )
    return null;
  let passed = 0;
  let failed = 0;
...
function compressDocker(content: string, lines: string[], totalLines: number): string | null {
  if (
    totalLines <= 3 ||
    !(lines.some((l) => /^CONTAINER\s+ID\s/.test(l)) || lines.some((l) => /^REPOSITORY\s+TAG\s/.test(l)))
  )
    return null;
  const h = lines[0];
...
function compressNpm(content: string, lines: string[], totalLines: number): string | null {
  if (
    !(
      lines.some((l) => /npm (ERR|WARN)/.test(l)) ||
      lines.some((l) => /^\s*\+?\s*[\w.-]+@/.test(l) && l.includes("added"))
    )
  )
    return null;
  ...
  const summaryLines = lines.filter((l) =>
    /^\s*(up to date|packages are looking|\d+ packages are|\d+ vulnerabilities?)/i.test(l),
  );
  const auditLines = lines.filter(
    (l) => /^\s*(added|removed|changed|found|\d+)/i.test(l) && /\b(audit|package|vulnerabilit)/i.test(l),
  );
  const parts: string[] = [];

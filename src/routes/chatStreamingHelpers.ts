import { logDeepseekSSE } from "../services/deepseekLogger.ts";
...
// ── Local MCP tool call extraction (from DeepSeek local_tool phase) ──

/**
 * Extract tool calls from SSE data containing `extra.local_mcp` in the delta.
 * DeepSeek sends tool calls in this format during the `local_tool` phase:
 *
 * ```json
...
export interface StreamProcessingState {
  ...
  /** Active JSON-Patch path for tracking which content field is being appended to */
  activePath: string | null;
  /** Set when upstream DeepSeek aborts the stream with an error (content filter,
...
export async function processStreamData(
  ...
  // Check for upstream DeepSeek error sent as SSE data chunk
  if (data.error) {
    const errMsg = typeof data.error === "string" ? data.error : data.error.message || JSON.stringify(data.error);
    logStore.addError(logId, `DeepSeek upstream SSE error: ${errMsg}`);
    ...
    state.upstreamError = `DeepSeek upstream error: ${errMsg}`;
    return "break_stream";
  }
  const deltaStatus = data.choices?.[0]?.delta?.status;
  if (deltaStatus === "error") {
    logStore.addError(logId, `DeepSeek stream delta returned error status`);
    logStore.updateEntry(logId, (entry) => {
      entry.finalResponse = entry.finalResponse || { finishReason: "", toolCallCount: 0, contentPreview: "" };
      entry.finalResponse.finishReason = "error";
    });
    state.upstreamError = "DeepSeek stream delta returned error status";
    return "break_stream";
  }
  ...
  if (deltaStatus === "finished") {
    ...
    if (deltaPhase === "local_tool") {
      ...
      if (ctx.deepseekLogFile && localToolCalls.length > 0) {
        logDeepseekSSE(ctx.deepseekLogFile, ctx.sseEventCount || 0, localToolCalls.length, localToolCalls);
      }
    }
  ...
  // DeepSeek native BATCH frame carries the running output token total
  if (data.o === "BATCH" && Array.isArray(data.v)) {
    for (const op of data.v) {
      if (op?.p === "accumulated_token_usage" && typeof op.v === "number") {
        state.completionTokens = op.v;
      }
    }
  }

  // Upstream toast error (e.g. unsupported_client_by_model) — surface instead of empty completion
  if (data.type === "error" && typeof data.content === "string" && data.content) {
    logStore.addError(logId, `DeepSeek upstream toast error: ${data.content}`);
    state.upstreamError = data.content;
    return "break_stream";
  }

  const deltaResult = extractDeltaContent(
    data,
    state.targetResponseId,
    state.currentThoughtIndex,
    state.reasoningBuffer,
    state.activePath,
  );
  const { vStr, foundStr, isThinkingChunk } = deltaResult;
  state.currentThoughtIndex = deltaResult.currentThoughtIndex;
  state.activePath = deltaResult.activePath;

  // JSON-Patch FINISHED status signals stream end
  if (deltaResult.streamFinished) {
    streamFinished = true;
  }
  ...
  const dsmlOpen = rawText.includes("<｜｜DSML｜｜invoke");
  const dsmlClose = rawText.includes("</｜｜DSML｜｜invoke");
  if (tagOpen || dsmlOpen) state.toolCallDepth++;
  if (tagClose || dsmlClose) state.toolCallDepth = Math.max(0, state.toolCallDepth - 1);

  // DSML container tracking: suppress content when inside a DSML tool-call block.
  // Upstream sends cumulative frames; detectCumulativeChunk strips the leading `<`,
  // so per-chunk tag detection never sees the full tag. Instead, detect the full DSML
  // marker `｜｜DSML｜｜`, which is preserved in the accumulated content (only the leading
  // `<` is lost) and never appears in normal text. Checking the full marker — rather
  // than a lone fullwidth bar — avoids suppressing legitimate content that happens to
  // contain a single `｜` (e.g. fullwidth table pipes in CJK output).
  const insideDsmlBlock = state.lastFullContent.includes("｜｜DSML｜｜");
  ...
  if (deltaCleaned && state.toolCallDepth === 0 && !insideDsmlBlock)
    state.lastFilteredFullContent = (state.lastFilteredFullContent || "") + deltaCleaned;
  ...
  if (cleanedText && state.toolCallDepth === 0 && !insideDsmlBlock) {

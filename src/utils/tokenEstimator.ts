/**
 * Token estimation utilities for deepseek-gateway.
 *
 * Uses content-aware heuristics for fallback token counting when DeepSeek's API
 ...
 *   3. Fallback when DeepSeek's usage field is absent from SSE chunks
 *
 * IMPORTANT: These are estimates. Actual token counts come from DeepSeek's API
 * response (chunk.usage.input_tokens / output_tokens). These heuristics
 * are only used BEFORE DeepSeek responds and as fallback.
 *
 * NOTE ON ACCURACY: Heuristic ratios may drift from actual DeepSeek tokenization.
 ...
 * These are empirically derived from DeepSeek model behavior.
 *
...
export function estimateTokens(
  text: string,
  options?: {
    tools?: Array<{ function?: { name: string; description: string; parameters?: object } }>;
    messageCount?: number;
  },
): number {
  if (!text) return 0;
...
export interface TokenBreakdown {
  ...
  /** Token count of content injected by deepseek-gateway (system prompts, formatting, tool instructions) */
  injectedOverhead: number;
  /** Total sent to DeepSeek (clientMessages + injectedOverhead) */
  totalSent: number;
}
 ...
 * @param finalPrompt - The full prompt sent to DeepSeek (after all injection)
 */

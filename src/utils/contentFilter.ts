...
import {
  DEEPSEEK_THINK_BLOCK_START,
  DEEPSEEK_THINK_TAG_PATTERN,
  type FilterResult,
  isThinkingLine,
} from "./thinkTagStripper.ts";
...
export function filterContent(raw: string): FilterResult {
  ...
  while (cursor < text.length) {
    ...
    const startMatch = remainder.match(DEEPSEEK_THINK_BLOCK_START);
    if (!startMatch) {
  ...
  text = text.replace(DEEPSEEK_THINK_TAG_PATTERN, " ");

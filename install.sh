#!/bin/bash
# ============================================================================
#  DeepSeek Gateway — One-Command Installer
# ============================================================================
#  Usage:
#    curl -sSL https://raw.githubusercontent.com/Chumdararith-LOU/deepseek_API_proxy/main/install.sh | bash
#
#  Clones the repo, installs Bun + dependencies, creates config, and
#  symlinks the CLI so you can run `dsg` from anywhere.
# ============================================================================

set -e

REPO_URL="https://github.com/Chumdararith-LOU/deepseek_API_proxy.git"
INSTALL_DIR="./Deepseek-API"
DEFAULT_PORT=26406
...
info "Setting up Deepseek-API..."
INSTALL_PATH="$(cd "$(dirname "$0")" 2>/dev/null && pwd)/Deepseek-API"
INSTALL_PATH="${INSTALL_PATH:-$(pwd)/Deepseek-API}"
...
link_cli() {
  ...
  ln -s "$PROJECT_ROOT/bin/dsg" "$target"
  ok "Linked $name -> $target"
}

link_cli "dsg"
link_cli "deepseek-gateway"
